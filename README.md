...
├── oar_mapping.json        # Contour name mapping
├── docs/                   # Documentation
...
## Quick Start
1. Place DICOM-RT data in `data/raw/case_XXXX/`
2. Edit `oar_mapping.json` for your site's contour naming conventions
3. Preprocess:
   ```bash
   python scripts/preprocess_dicom_rt_v2.2.py --mapping_file ./oar_mapping.json
   ```
4. Verify outputs with `notebooks/verify_npz.ipynb`
5. Train the model:
   ```bash
   python scripts/train_dose_ddpm_v2.py --epochs 200
   ```

**Note:** Scripts auto-detect data paths. Use `--input_dir`, `--output_dir`, or `--data_dir` to specify custom locations.
