"""
Generate publication-ready figures for DDPM v2.3 preliminary experiment.

DoseDDPM with SimpleUNet3D backbone, cosine schedule, 1000 timesteps,
DDIM 50-step sampling. MSE + physics penalty loss (no combined loss).

Created: 2026-03-07
Experiment: ddpm_v23_seed42 (1-seed preliminary scout)
Result: NEGATIVE — DDPM dramatically underperforms baseline on all metrics.
"""
...
def _load_npz_volumes_ddpm(case_id: str):
    """Load GT and DDPM prediction NPZ volumes.

    DDPM predictions use 'pred_dose' key instead of baseline's 'dose'.
    """
    gt_path = base_figs.DATA_DIR / f'{case_id}.npz'
    pred_path = base_figs.PRED_DIR / f'{case_id}_pred.npz'
    ...
    ct = gt_npz['ct']
    gt_dose = gt_npz['dose'] * base_figs.RX_DOSE_GY
    pred_dose = pred_npz['pred_dose'] * base_figs.RX_DOSE_GY
    masks = gt_npz['masks']
...
def configure_paths():
    """Set module-level path variables for DDPM v2.3 experiment."""
    base_figs.SEED_LABEL = 'DDPM v2.3 (seed 42)'
    ...
    base_figs.EVAL_JSON = base_figs.PRED_DIR / 'ddpm_evaluation_results.json'
    base_figs.FIG_DIR = PROJECT_ROOT / 'runs' / 'ddpm_v23' / 'figures'


def main():
    parser = argparse.ArgumentParser(
        description='Generate figures for DDPM v2.3 preliminary experiment.')
    ...
    print('=' * 70)
    print(f'  DDPM v2.3 Preliminary Figure Generation')
    print('=' * 70)
    print()
    ...
    if rep_case:
        ...
        volumes = _load_npz_volumes_ddpm(rep_case)
