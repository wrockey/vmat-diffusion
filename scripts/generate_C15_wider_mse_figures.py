"""
Generate publication-ready figures for C15 architecture scout experiment.

Wider BaselineUNet3D (base_channels=50) with MSE-only loss (seed42).
Part of architecture comparison study (#53).

Created: 2026-03-03
Experiment: C15_wider_mse_seed42
"""
...
def configure_c15_paths():
    """Set module-level path variables for C15 experiment."""
    base_figs.SEED_LABEL = 'C15 Wider Baseline (bc=50) + MSE (seed 42)'
    ...
    base_figs.FIG_DIR = PROJECT_ROOT / 'runs' / 'C15_wider_mse' / 'figures'


def main():
    parser = argparse.ArgumentParser(
        description='Generate figures for C15 Wider Baseline MSE experiment.')
    ...
    print('=' * 70)
    print(f'  C15 Wider Baseline (bc=50) + MSE Figure Generation')
    print('=' * 70)
    print()
