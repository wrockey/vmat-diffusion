"""
Generate publication-ready figures for combined_loss_pilot experiment.

5-component loss (MSE + Gradient + DVH + StructureWeighted + AsymmetricPTV)
with Kendall 2018 uncertainty weighting. Single seed (42) pilot run.
...
Created: 2026-02-28
Experiment: combined_loss_pilot (seed42, 5-component + uncertainty weighting)
"""
...
def configure_combined_loss_pilot_paths():
    """Set module-level path variables for the combined loss pilot experiment."""
    base_figs.SEED_LABEL = 'combined loss pilot (seed 42)'

    exp_name = 'combined_loss_pilot_seed42'

    # Run dir — post path-fix, should be in PROJECT_ROOT/runs/
    ...
    # Eval JSON — predictions in PROJECT_ROOT/predictions/
    ...
    base_figs.FIG_DIR = PROJECT_ROOT / 'runs' / 'combined_loss_pilot' / 'figures'


def main():
    parser = argparse.ArgumentParser(
        description='Generate figures for combined loss pilot experiment.')
    parser.add_argument('--case', type=str, default=None,
                        help='Case ID for single-case figures (default: below-median MAE case)')
    args = parser.parse_args()

    # Configure paths for combined loss pilot
    configure_combined_loss_pilot_paths()
    ...
    print('=' * 70)
    print(f'  Combined Loss Pilot Figure Generation')
    print('=' * 70)
    print()
