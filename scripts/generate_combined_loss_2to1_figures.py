"""
Generate publication-ready figures for combined loss 2:1 weight tuning experiment.

BaselineUNet3D with full combined loss (MSE + Gradient + DVH + Structure +
Asymmetric PTV), uncertainty weighting. Asymmetric underdose:overdose = 2:1
(tuned down from 3:1 pilot to reduce D95 overdose).

Created: 2026-03-03
Experiment: combined_loss_2to1_seed42
"""
...
def configure_paths():
    """Set module-level path variables for combined loss 2:1 experiment."""
    base_figs.SEED_LABEL = 'Combined Loss 2:1 (seed 42)'
    ...
    base_figs.FIG_DIR = PROJECT_ROOT / 'runs' / 'combined_loss_2to1' / 'figures'


def main():
    parser = argparse.ArgumentParser(
        description='Generate figures for combined loss 2:1 weight tuning experiment.')
    ...
    print('=' * 70)
    print(f'  Combined Loss 2:1 Figure Generation')
    print('=' * 70)
    print()
