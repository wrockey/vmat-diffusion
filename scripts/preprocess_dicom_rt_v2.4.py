...
Version: 2.4.0 - Multi-protocol support (PTV50.4 + SDF fix)

Changes from v2.3.0:
- Added PTV50.4 as channel 8 (9 structures total) for multi-protocol training.
  Enables 3 plan types: PTV70+PTV56, PTV70+PTV56+PTV50.4, PTV70+PTV50.4 (#66)
- CRITICAL FIX: SDF for absent structures now returns 0.0 (not +1.0).
  Creates a three-way signal: negative=inside, positive=outside, zero=absent.
  The old convention (+1.0) was indistinguishable from "far from real structure"
  and caused dose hallucination for absent PTVs (#67)
- Only PTV70 is required; PTV56 gate removed (--relax_filter deprecated)
- Case type detection updated: sib_2level, sib_3level, nodal_only, single_rx
- DVH validation added for PTV56 and PTV50.4 D95 when present
- Batch summary includes case-type distribution counts
...
Key Features:
- 9 structure channels including PTV50.4 for multi-protocol prostate VMAT
- Signed Distance Fields (SDFs) with absent-structure convention (SDF=0.0)
- FULL beam geometry from RP file (MLC, dose rate, jaws) for all arcs
- Improved truncation quantification
- HU clipping detection
- DVH validation per case (PTV70, PTV56, PTV50.4)

.npz File Structure (v2.4.0):
- ct: (Y, X, Z) float32 - CT normalized [0,1], native resolution cropped
- dose: (Y, X, Z) float32 - Dose normalized to Rx, on CT grid cropped
- masks: (9, Y, X, Z) uint8 - Binary structure masks, native grid cropped
- masks_sdf: (9, Y, X, Z) float32 - Signed distance fields [-1,1]
  (0.0 everywhere for absent structures)
- constraints: (13,) float32 - Prescription + OAR constraints
- metadata: dict - Case info, spacing, crop, beam geometry, validation
- beamN_mlc_a: (n_cp, n_leaves) float32 - MLC bank A positions per control point
- beamN_mlc_b: (n_cp, n_leaves) float32 - MLC bank B positions per control point
  (N = 0, 1, 2, ... for each arc in the plan)

Assumptions: See docs/preprocessing_assumptions.md

Usage: python preprocess_dicom_rt_v2.4.py [flags]
"""
...
# Crop parameters (v2.3+, unchanged in v2.4)
...
def determine_case_type(masks):
    ...
    Plan types:
        sib_3level:  PTV70 + PTV56 + PTV50.4 (prostate + SV + nodes)
        sib_2level:  PTV70 + PTV56 (prostate + SV, no nodes)
        nodal_only:  PTV70 + PTV50.4 (prostate + nodes, no SV)
    ...
    ptv50_exists = masks[8].sum() > 0 if masks.shape[0] > 8 else False

    if ptv56_exists and ptv50_exists:
        case_type = 'sib_3level'
    elif ptv56_exists:
        case_type = 'sib_2level'
    elif ptv50_exists:
        case_type = 'nodal_only'
    else:
        case_type = 'single_rx'
...
def compute_sdf(binary_mask, spacing_mm=(1.0, 1.0, 2.0), clip_mm=DEFAULT_SDF_CLIP_MM):
    ...
    if binary_mask.sum() == 0:
        # Empty mask — structure does not exist in this case.
        # Return 0.0 everywhere: distinct from inside (<0) and outside (>0).
        # A real structure never has SDF=0.0 uniformly (always has boundary).
...
def validate_preprocessed_data(ct_volume, dose_volume, masks_resampled, prescription_dose,
    ...
    # Registration check (union of all present PTVs)
    any_ptv = (masks_resampled[0] > 0) | (masks_resampled[1] > 0)
    if masks_resampled.shape[0] > 8:
        any_ptv = any_ptv | (masks_resampled[8] > 0)
    ...
    truncation_info = {}
    critical_structures = [0, 1, 3, 4, 8]  # PTV70, PTV56, Rectum, Bladder, PTV50.4
...
def preprocess_dicom_rt(plan_dir, output_dir, structure_map, relax_filter=False,
    ...
    v2.3/v2.4 pipeline: keeps CT and masks at native resolution, resamples only
    ...
    Args:
        ...
        relax_filter (bool): Deprecated in v2.4 (PTV56 no longer required).
    ...
    try:
        ...
        ptv50_sum = masks[8].sum() if masks.shape[0] > 8 else 0
        print(f"PTV70 sum: {ptv70_sum}, PTV56 sum: {ptv56_sum}, PTV50.4 sum: {ptv50_sum}")
        
        if ptv70_sum == 0:
            warnings.warn(f"Skipping {plan_dir}: Missing PTV70 (required)")
            return None

        # v2.4: PTV56 no longer required — multi-protocol support.
        # PTV50.4 (channel 8) and PTV56 (channel 1) are both optional.
        ...
        ptv50_cropped = masks_resampled[8].sum() if masks_resampled.shape[0] > 8 else 0
        print(f"Cropped PTV70 sum: {masks_resampled[0].sum()}, PTV56 sum: {masks_resampled[1].sum()}, PTV50.4 sum: {ptv50_cropped}")

        # =================================================================
        # DVH validation: check D95 for all present PTVs
        # =================================================================
        dvh_validation = {}
        ptv_checks = [
            (0, 'PTV70', 64.0, 70.0),
        ...
        for ch, name, d95_threshold, rx in ptv_checks:
            if ch >= masks_resampled.shape[0]:
                continue
            ptv_mask_bool = masks_resampled[ch] > 0
            if ptv_mask_bool.sum() == 0:
                continue
            ptv_dose_gy = dose_volume[ptv_mask_bool] * normalization_dose
            d95_gy = float(np.percentile(ptv_dose_gy, 5))  # D95 = 5th percentile
            dvh_validation[name] = {'d95_gy': d95_gy, 'rx_gy': rx, 'n_voxels': int(ptv_mask_bool.sum())}
            print(f"{name} D95 validation: {d95_gy:.1f} Gy (target >= {d95_threshold:.1f} Gy)")
            if d95_gy < d95_threshold:
                warnings.warn(f"WARNING: {name} D95 = {d95_gy:.1f} Gy < {d95_threshold:.1f} Gy — "
                             f"possible pipeline artifact for {plan_id}")
            # Check dose coverage: warn if >5% of PTV voxels have zero dose
            zero_dose_frac = float((ptv_dose_gy < 0.1).sum()) / ptv_mask_bool.sum()
            if zero_dose_frac > 0.05:
                warnings.warn(f"WARNING: {zero_dose_frac:.1%} of {name} voxels have near-zero "
        ...
        if not skip_plots:
            ...
            if masks_resampled.shape[0] > 8 and masks_resampled[8][:, :, mid].sum() > 0:
                axs[0, 2].contour(masks_resampled[8][:, :, mid], colors='yellow', linewidths=1)
            axs[0, 2].set_title('PTV70/56/50.4')
...
def batch_preprocess(input_base_dir, output_dir, mapping_file='oar_mapping.json',
    ...
    # Compute case-type distribution from processed NPZ files
    case_type_counts = {'sib_2level': 0, 'sib_3level': 0, 'nodal_only': 0, 'single_rx': 0}
    case_type_list = {}
    for npz_path in processed:
        try:
            npz = np.load(npz_path, allow_pickle=True)
            meta = npz['metadata'].item()
            ct = meta.get('case_type', {}).get('type', 'unknown')
            case_type_counts[ct] = case_type_counts.get(ct, 0) + 1
            case_type_list[meta.get('case_id', os.path.basename(npz_path))] = ct
        except Exception:
            pass
    ...
    for ct_name, ct_count in sorted(case_type_counts.items()):
        if ct_count > 0:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Preprocess DICOM-RT to .npz for VMAT diffusion model (v2.4.0 - Multi-Protocol)")
    ...
    parser.add_argument("--relax_filter", action="store_true",
                        help="Deprecated in v2.4 (PTV56 no longer required). Kept for CLI compatibility.")
