"""
Generate publication-ready figures for C13 architecture scout experiment.

BottleneckAttnUNet3D with MSE-only loss (seed42). Part of architecture
comparison study (#53).

Created: 2026-03-02
Experiment: C13_bottleneck_mse_seed42
"""
...
def configure_c13_paths():
    """Set module-level path variables for C13 experiment."""
    base_figs.SEED_LABEL = 'C13 BottleneckAttn + MSE (seed 42)'

    exp_name = 'C13_bottleneck_mse_seed42'

    base_figs.RUN_DIR = PROJECT_ROOT / 'runs' / exp_name

    # Prefer merged metrics.csv at run root, then search version dirs
    merged = base_figs.RUN_DIR / 'metrics.csv'
    if merged.exists():
        base_figs.METRICS_CSV = merged
    else:
    ...
    base_figs.FIG_DIR = PROJECT_ROOT / 'runs' / 'C13_bottleneck_mse' / 'figures'


def main():
    parser = argparse.ArgumentParser(
        description='Generate figures for C13 BottleneckAttn MSE experiment.')
    ...
    print('=' * 70)
    print(f'  C13 BottleneckAttn + MSE Figure Generation')
    print('=' * 70)
    print()
